...
class Tabelle {
    ...
    public function setAnzLabels($anzahl) {
        if (!is_numeric($anzahl)) {
            die("setAnzLabels erwartet einen numerischen Wert");
        }
        $this->anz_labelzeilen = $anzahl;
    }

    public function getAnzLabels($anzahl) {
        return $this->anz_labelzeilen;
    }
    ...
    private function validateCoordinates($x, $y) {
        $int_x = intval($x);
        $int_y = intval($y);
        if (($this->y_max < $int_y) || ($this->x_max < $int_x)) {
